...
Tool:
	...
	getSimilarityScores2Reports.py：从历史角度，计算相似的issue report，以致推荐出使用的API以及设计的源代码文件等
	getFeatureLocation_result.py：将特征定位的结果进行汇总，得到最后的结果
	
	getAPISrcfileScores.py：用于数据准备工作，得到API description与feature location结果相关的源文件s的相似度分数，便于后期实验
	getAPIdscpScors.py：用于数据准备工作，得到API description与issue report中的相似度分数，便于后期实验
	...
	数据准备：
	extrPatchInfo.py：issue解决后，从提交的Attachments（diff）即 .patch文件中提取有用的信息（使用的API，涉及的文件等）
