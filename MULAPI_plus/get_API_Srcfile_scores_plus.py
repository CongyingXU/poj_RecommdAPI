...
7366*1.85244297981
7366*15（一个项目10亿次）
时间上消耗不起，7366不可行，这个思路估计是废了，要么就不是7366个参数
...
def computeSimilarScores( RelatedSrcfileinfo_dict , parainfo_list ):
    ...
    para_result_dict =  {}#用于保存已计算过的参数分数，再次遇到时直接赋值，不再计算，以减少调用次数     格式：{'参数信息'：分数}
    begin0 = time.time()
    num=0
    for para in parainfo_list:
            print  time.time()  - begin0
            begin0 = time.time()
            print num
            num = 0
            ...
            para_string = ''  #将信息从列表转成字符串格式
            for pa in para:
                para_string = para_string + pa
            
            #print para
            if len(para)<2:#api 无参数
                max_para_result = 0
                text3_result_list.append(max_para_result)
                continue
            elif para_string in para_result_dict:#同样的APi参数之前已经计算过
                max_para_result = para_result_dict[para_string]
                text3_result_list.append(max_para_result)
                continue
            else:
                ...
                while(2*i + 1<=len(parainfo)/2):#API 的形式参数  不止一个
                    ...
                    for ele in text3_list:# 依次对比每一组变量信息,变量信息也不止一个
                    ...
                    i = i+1
                para_result_dict[para_string] = max_para_result
                text3_result_list.append(max_para_result)
                
        
    return text3_result_list
