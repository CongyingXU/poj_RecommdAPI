...
"""
#优化测试版
def getFinal_Result(VersionHis_result, Similarreports_result , Structure_result ,Reporter_result, weights):
    Final_Result={}
    for key in Structure_result:
        for key0 in Structure_result[key]:
            if not VersionHis_result[key].has_key(key0):
                VersionHis_result[key][key0] = 0.0
                
            if not Similarreports_result[key].has_key(key0):
                Similarreports_result[key][key0] = 0.0
                
            if not Reporter_result[key].has_key(key0):
                Reporter_result[key][key0] = 0.0
    
    
    for key in  Structure_result:
        result_dict={}
        for key0 in Structure_result[key]:
                        result_dict[key0] = weights[0]*VersionHis_result[key][key0]+weights[1]*Similarreports_result[key][key0]
                        +weights[2]*Structure_result[key][key0]+weights[3]*Reporter_result[key][key0]
...
def write(Final_Result,evaluate_result):
    ...
    f.save('Output/Amlg_HadoopCommon_FeaturnLocation_result.xls')
