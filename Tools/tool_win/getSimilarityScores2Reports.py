...
def getnewreportInfo():
    oldReports_dir='/Users/apple/Desktop/open_issuekey/Axis2.xlsx'
    workbook = xlrd.open_workbook(oldReports_dir,'r')
    sheet1 = workbook.sheet_by_name('Axis2')
    #print sheet1.cell(6,28).value.encode('utf-8')
    All_newreportinfo=[]
    
    data = range(1,14)# + range(904,1004)
    for i in data:
        print i
        newReportSummary=sheet1.cell(i,0).value.encode('utf-8')
        newReportDescription=sheet1.cell(i,36).value.encode('utf-8')
        newReportSummaryAndDescription = newReportSummary + ' ' +newReportDescription
        newReportComponent=sheet1.cell(i,24).value.encode('utf-8').split(',')
        newReportReporter=sheet1.cell(i,14).value.encode('utf-8')
        newReportPriority=sheet1.cell(i,11).value.encode('utf-8')
        newReportIssueKey=sheet1.cell(i,1).value.encode('utf-8')
        newReportLinkedissue=sheet1.cell(i,1).value.encode('utf-8').split(',')
...
def getOldreportsInfo():
    oldReports_dir='Input/Axis2.xlsx'
    workbook = xlrd.open_workbook(oldReports_dir,'r')
    ...
    for i in range(4,14):####下面的字典中，也要改范围
        texts0=[]
...
def getPathinfo():
    ...
    with open("Input/Axis2_Attachments_PatchInfo.csv","r") as csvfile:
...
def getIssueKey_UsedAPIinfo():
    ...
    workbook = xlrd.open_workbook(r'Input/Axis2_issuekeys_UsedAPI.xls')
...
def main_API():
    ...
    #begin = time()
    weights = [1, 0.8, 1.3877787807814457e-16, 1.3877787807814457e-16, 1, 1, 1, 1.3877787807814457e-16, 1, 1, 0.6000000000000001, 1, 1, 1, 1, 1, 1]
