...
def getnewreportInfo():
        ...
        newReportSummary= Issue.Summary
        newReportDescription= Issue.Description
        newReportSummaryAndDescription = newReportSummary + ' ' +newReportDescription
        newReportComponent= Issue.Component 
        newReportReporter= Issue.Reporter
        newReportPriority= Issue.Priority
        newReportIssueKey= Issue.Issuekey
        newReportLinkedissue= Issue.Linkedissue
...
def getOldreportsInfo():
    #import os
    #print os.getcwd()
    oldReports_dir='app0/Computing_recommendation/Input/'+ poj+'.xlsx'
    ...
    for i in range(4,10):####下面的字典中，也要改范围
        texts0=[]
    ...
    for OldReportComponentList in OldReportsComponent:
        if OldReportComponentList[0]=='' or OldReportComponentList[0]=='':
                ...
                for x in newReportComponent:
                    if x in OldReportComponentList:
                        count=count+1
                if (math.sqrt(len(newReportComponent))*math.sqrt(len(OldReportComponentList)))>0:
                    result0=count/(math.sqrt(len(newReportComponent))*math.sqrt(len(OldReportComponentList)))
                else:
                    result0=0 
        Comp_result.append(result0)
...
def getPathinfo():
    ...
    with open("app0/Computing_recommendation/Input/"+ poj+"_Attachments_PatchInfo.csv","r") as csvfile:
...
def getIssueKey_UsedAPIinfo():
    ...
    workbook = xlrd.open_workbook(r'app0/Computing_recommendation/Input/'+ poj+'_issuekeys_UsedAPI.xls')
...
def get_similar_issues(issue):
    ...
    for all_result in All_result:
        ...
        Result_list=[ ele[0] for ele in result_list ]
        Result_dict[newReportIssueKey] = Result_list
        
    


    return Result_dict[issue.Issuekey]
""" 
